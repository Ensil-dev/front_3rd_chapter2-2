...
import { vi } from 'vitest' // vi를 별도로 import
import {
  calculateItemTotal,
  getMaxApplicableDiscount,
  updateCartItemQuantity,
} from '../../refactoring/hooks/utils/cartItem'
import { calculateCartTotal } from '../../refactoring/hooks/utils/services/cartCalculationService'
...
describe('basic > ', () => {
  ...
  describe('useCart > ', () => {
    ...
    test('제품 수량을 업데이트해야 합니다', () => {
      ...
      expect(result.current.cart).toHaveLength(1)
      console.log('Cart after adding:', result.current.cart)

      act(() => {
        result.current.updateQuantity(testProduct.id, 5)
      })

      console.log('Cart after update:', result.current.cart)
      expect(result.current.cart).toHaveLength(1)
    ...
    test('합계를 정확하게 계산해야 합니다', () => {
      ...
      console.log('Selected coupon:', result.current.selectedCoupon)
